...
管理端「图片推送」支持指定群、多个群和动态所有群的手动/自动图片推送，并提供固定相同、固定不同、随机相同、随机不同四种分配方式。详细规则和 Telegram 命令见 [主动图片推送](docs/image-push.md)。

管理端「图片图库」用于浏览、编辑、批量维护和刷新已添加的 Pixiv/手动图片；媒体读取、删除清理和存储安全边界见 [图片图库管理](docs/illustration-gallery.md)。

## 智能群管

群内使用 `/guard` 打开管理面板，或从 Web「群组管理」工作区进入「智能群管」。工作区统一提供群组配置、验证、反广告刷屏、警告禁言封禁、群规欢迎、自动回复、定时公告、观测成员目录、举报复核和可选文本/图片 AI 审核。机器人入群审批默认开启，机器人违规检测默认关闭；普通成员的旧配置保持不变。

配置示例、命令列表、外部网关与 webhook 设置见 [智能群管使用与部署](docs/group-guard.md)。
