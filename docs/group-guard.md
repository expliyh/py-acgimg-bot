...
群管提供规则审核、入群验证、防刷屏与突袭、警告处罚、群运营、举报复核和可选 AI 审核。Telegram 与 Web 共用数据库配置。机器人入群审批默认开启；机器人违规检测默认关闭，普通成员的既有入群验证保持不变。
...
4. 将机器人设置为群管理员。删除消息需要删除权限，验证/禁言/封禁需要限制成员权限，入群申请需要邀请权限，置顶需要置顶权限。
   机器人入群审批还需要 `can_restrict_members` 才能在 supergroup 保存并执行逐成员限制；缺少权限时会保留待审批记录，不会把机器人误判为已批准。
5. 在群里发送 `/guard`，或者在 Web「群组管理」工作区打开「智能群管」。Web 群组管理将基础配置、智能群管、群成员和审核日志合并在同一页面；历史上直接打开 `/guard` 或 `/groups/:id/guard` 的链接会重定向到对应群组工作区。概览显示机器人当前权限。
...
/guard set bot_join_approval_enabled on
/guard set bot_moderation_enabled on
...
机器人入群后会立即保存原有权限并限制发言，直到群管理员在群内按钮或 Web 复核页选择「批准发言」；「移出机器人」会执行临时封禁后解除，使机器人可以再次申请入群。审批没有超时，机器人管理员身份、管理员主动恢复发言权限或通过入群申请批准都视为明确批准。主群管机器人始终豁免。只有 supergroup 支持逐成员限制（见 [Telegram `restrictChatMember`](https://core.telegram.org/bots/api#restrictchatmember)）；基础群会记录失败并保持待处理。关闭 `bot_join_approval_enabled` 会通过 `bot_release` 任务恢复所有存量机器人的原权限，任务完成后清理批准状态，重新开启不会沿用旧批准。
...
- `bot_moderation_enabled` 关闭时，机器人跳过关键词、规则、反刷屏、AI 和自动处罚；手工举报与管理员手工操作不受影响。开启后，已批准且非管理员/非豁免机器人的行为沿用普通成员流程，AI 执行前会再次检查审批状态。
...
群管 API 根路径为 `/api/groups/{group_id}/guard`，包含设置、规则、成员状态、成员目录、操作、群运营内容、复核、日志、任务和统计。模型配置为 `/api/guard-ai`。请求结构在 FastAPI OpenAPI 中可见。

Web 群成员目录使用 `GET /api/groups/{group_id}/guard/members`。目录合并机器人已观测到的消息发送者、管理员及群管记录，并支持搜索、角色/状态筛选、排序和分页；`total` 是已观测成员数，不是 Telegram 普通成员全量数。若机器人可连接，响应会附带 Telegram 成员总数，无法获取时为 `null`。成员行可打开详情抽屉并执行单成员处罚，所有 Telegram 操作仍遵循权限检查、幂等键和不确定结果保护。

机器人复核使用 `bot_join` 类型；决策值为 `approve_bot`（批准发言）或 `reject_bot`（临时封禁后解除并移出）。成员查询响应中的 `bot_approval` 返回当前机器人代次、状态、原始权限快照和复核关联；关闭入群审批后的 `bot_release` 任务必须在任务页核查完成或不确定结果。
